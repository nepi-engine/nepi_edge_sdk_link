...
TEMPLATE_FILE=$1
TARGET_DIR='.'
EXCLUSIONS='*generate_license_headers.sh* *frozen*' # frozen package is separately licensed by original author
OWNER='Numurus, LLC <https://www.numurus.com>'
PROJECT='nepi_edge_sdk_link'
PROJECT_URL='https://github.com/numurus-nepi/nepi_edge_sdk_link'

# Create a dummy Python file to generate the LICENSE file
touch ${TARGET_DIR}/license_dummy.py

# Generate headers
python3 -m licenseheaders -v -v -v -t ${TEMPLATE_FILE} -d ${TARGET_DIR} -x ${EXCLUSIONS} -o "${OWNER}" -n ${PROJECT} -u ${PROJECT_URL} -cy
